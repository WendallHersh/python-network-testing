...
def get_network_details():
    """Retrieve the local IP, gateway, and subnet mask using system commands."""
    try:
...
def _get_network_details_linux():
    ...
    try:
        local_ip = subprocess.check_output("hostname -I", shell=True, text=True).split()[0]
        gateway = subprocess.check_output("ip route | grep default", shell=True, text=True).split()[2]
        subnet_mask = "255.255.255.0"  # Default for simplicity; can parse `ifconfig` for more accuracy.
        return local_ip, gateway, subnet_mask
    except Exception as e:
